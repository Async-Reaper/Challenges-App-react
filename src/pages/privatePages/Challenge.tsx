...
const Challenge: FC = () => {
  ...
  const { data: challenge, isLoading } = postApi.useGetChallengeByIdQuery(
    params.id!
  );
  const { data: challengeMembers } = postApi.useGetChallengeMembersQuery(
    params.id!
  );
  ...
  return (
    <MainWrapper dataTestid="challengeById-page">
      ...
      {challengeMembers && (
        <ChallengeMembers challengeMembers={challengeMembers} />
      )}
    </MainWrapper>
  );
};
